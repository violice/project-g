...
export const Tuner = () => {
  ...
  return (
    <div className="bg-white rounded-xl p-6 shadow-sm border border-slate-200">
      ...
      <p className="mb-4 text-sm text-slate-500">{getStatusText()}</p>

      {currentResult?.note && (
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <span className="text-6xl font-bold text-slate-800">{currentResult.note.name}</span>
            <span
          ...
          <div className="mt-4">
            <div className="relative h-4 rounded-full bg-slate-200">
              ...
              />
              <div className="absolute left-1/2 top-0 h-full w-0.5 -translate-x-1/2 bg-slate-400" />
            </div>
          </div>
        </div>
      )}
